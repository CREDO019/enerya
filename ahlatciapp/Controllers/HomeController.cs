...
namespace ahlatciapp.Controllers
{
	public class HomeController : Controller
	{
        ...
        [Route("ÖdemeSayfası")]
        public IActionResult ÖdemeSayfası()
        {
        ...
        [Route("Kbasarili")]
        public IActionResult kbasarili()
        {
            return View();
        }
        [Route("KayitForm")]
		...
		public async Task<IActionResult> Kayit(MainModel model)
		{
            ...
            return RedirectToAction("kbasarili", "Home");
		...
		[HttpGet("borcsorgu")]
        public async Task<IActionResult> borcGetir([FromQuery] string tcno)
        {
            var abone = await context.AboneBilgi.FirstOrDefaultAsync(item => item.TcNo == tcno);
            var aboneId = abone.Id;

            var faturalar = await context.Faturalar
                .Where(item => item.AboneKsBilgiId == aboneId)
                .Select(fatura => new
                {
                    fatura.Id,
                    fatura.M3,
